...
The gate checks syntax, unit behavior, MCP startup/schema, built output, manifests, required files, personal paths, and key-like text. It does not make provider API calls.

## Live API test

`npm run test:e2e` launches two DeepSeek workers and requires:

- `DEEPSEEK_API_KEY` in the environment
- a working `deepseek` Codex profile
- a small amount of API credit

The test passes only when both exact responses return and both rollouts attest provider `deepseek` and model `deepseek-flash`.
